...
build_version() {
    ...
    # Deploy with mike - use different commands for local vs Vercel
    ...
    if [ -n "$alias" ] && [ -n "$title" ]; then
        $MIKE_CMD deploy "$version" "$alias" --title="$title"
    elif [ -n "$alias" ]; then
        $MIKE_CMD deploy "$version" "$alias"
    elif [ -n "$title" ]; then
        $MIKE_CMD deploy "$version" --title="$title"
    else
        $MIKE_CMD deploy "$version"
    fi
...
echo "🎉 Vercel versioned build completed successfully!"
